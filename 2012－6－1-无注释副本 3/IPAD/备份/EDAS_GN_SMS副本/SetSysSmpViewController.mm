...
- (void) initUI1 :(NSInteger) index {
	...
	int m_sensid = index;
	printf("sens_id=%d \n", m_sensid);
	int smp[]={1,10,20,50,100,200,500};
	int i = 0;
	for(i=0;i<7;i++)
	{
...
- (void)viewDidLoad
{
	...
	radioLine =[[ UICheckButton alloc ] initWithFrame : CGRectMake ( 400 , 300 , 160 , 32 )];
	...
	radioMin =[[ UICheckButton alloc ] initWithFrame : CGRectMake ( 530 , 300 , 160 , 32 )];
	...
	comboSamp = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 450 , 215 , 160 , 32)];
	//comboSamp.dropMaxHeigth  = 32*5;

	NSArray *coData = [[NSArray alloc] initWithObjects:@"1 HZ",@"10 HZ",@"20 HZ",@"50 HZ",@"100 HZ", @"200 HZ",@"500 HZ", nil];
	...
	comboSens= [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 450 , 180 , 160 , 30 )];
...
- (IBAction)btnOkClick:(id)sender {
	...
	if(sleIndex == 0) {samp = 1; samp_id = 1;}
	else if(sleIndex == 1) {samp = 10; samp_id = 2;}
	else if(sleIndex == 2) {samp = 20; samp_id = 3;}
	else if(sleIndex == 3) {samp = 50; samp_id = 4;}
	else if(sleIndex == 4) {samp = 100; samp_id = 5;}
	else if(sleIndex == 5) {samp = 200; samp_id = 7;}
	else if(sleIndex == 6) {samp = 500; samp_id = 8;}
	int i = 0;
	for(i=0;i<16;i++)
	{
