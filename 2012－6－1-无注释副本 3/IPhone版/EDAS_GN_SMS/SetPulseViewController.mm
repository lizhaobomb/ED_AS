...
- (void)viewDidLoad
{
	...
	//-------------------------------------------- 
	UILabel *labAmpType = [[UILabel alloc] initWithFrame : CGRectMake ( 50 , 275 , 180 , 30 )];
	labAmpType.text = @"幅度类型：";
	[scrollView addSubview:labAmpType];[labAmpType release];
	
	cmboAmpType= [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 165 , 275 , 200 , 28 )];
	...
	//-------------------------------------------- 
	UILabel *labAmp = [[UILabel alloc] initWithFrame : CGRectMake ( 50 , 310 , 180 , 30 )];
	labAmp.text = @"脉冲幅度：";
	[scrollView addSubview:labAmp];[labAmp release];
	
	txtAmp = [[UITextField alloc] initWithFrame:CGRectMake(165, 310, 200, 30)];
	txtAmp.borderStyle =  UITextBorderStyleRoundedRect;
	txtAmp.delegate =self;
	[scrollView addSubview:txtAmp];
	
	labUnit = [[UILabel alloc] initWithFrame : CGRectMake ( 365 , 310 , 80 , 30 )];
	labUnit.text = @"(count)";
	[scrollView addSubview:labUnit];[labUnit release];
	
	//-------------------------------------------- 
	UILabel *labDur = [[UILabel alloc] initWithFrame : CGRectMake ( 50 , 345 , 180 , 30 )];
	labDur.text = @"脉冲宽度：";
	[scrollView addSubview:labDur];[labDur release];
	
	txtDur = [[UITextField alloc] initWithFrame:CGRectMake(165, 345, 200, 30)];
...
- (IBAction)btnOKClick:(id)sender {
	...
	for(int i=0;i<(frm.head.length+4)/2-1;i++)
		frm.chk_sum-=p[i];
	frm.chk_sum-=frm.dur;
	
	memcpy((void *)&m_cmd[4],(void *)&frm,frm.head.length+6);
