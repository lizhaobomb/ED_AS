...
- (void)viewDidLoad
{
	...
	//----------------------------------------------------------------------------  
	UILabel *labDrqNum = [[UILabel alloc] initWithFrame : CGRectMake ( 50 , 275 , 180 , 30 )];
	labDrqNum.text = @"频点数：";
	[scrollView addSubview:labDrqNum];[labDrqNum release];
	
	
	txtFrqNum = [[UITextField alloc] initWithFrame:CGRectMake(165, 275, 200, 30)];
	...
	radioAtten =[[ UICheckButton alloc ] initWithFrame : CGRectMake ( 50 , 310 , 160 , 32 )];
	...
	radioAmp =[[ UICheckButton alloc ] initWithFrame : CGRectMake ( 165 , 310 , 200 , 32 )];
	...
	tableSines = [[SineTable alloc] initWithFrame: CGRectMake ( 50 , 345 , 330 , 100 )];
	...
	*/
	...
	UILabel *labFraModel = [[UILabel alloc] initWithFrame : CGRectMake ( 50 , 458 , 150 , 30 )];
	labFraModel.text = @"频点模板：";
	[scrollView addSubview:labFraModel];[labFraModel release];
	
	cmbFraModel = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 130 , 460 , 150 , 28 )];
	...
	*/
	...
	btnFraModel.frame = CGRectMake( 285 , 458 , 90 , 30 );
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
	 ...
	 if(textField == txtFrqNum){
		 ...
		 if(aa > 100 || aa < 0)	{
			 UIAlertView *av = [[[UIAlertView alloc] initWithTitle:@"提示" message:@"请输入大于 0 小于 100 的整数" 
