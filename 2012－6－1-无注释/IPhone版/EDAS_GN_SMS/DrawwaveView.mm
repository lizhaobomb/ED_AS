...
-(void) dsp_data: (long *)pin : (int) l//{//(long * pin,CHANNEL *p,int l)
{
    int i;
    int dspoints;
	float offst;
	...
	if(p->dsp.unit==1){
		...
		factor=1.f;
	else  if(p->dsp.unit>4 && (p->chn_id>=6 && p->chn_id<12) )//◊‘∂®“Âµƒµ•Œª,œﬁ÷∆‘⁄∏®÷˙Õ®µ¿
...
-(void) OnScantime{
	...
	UIActionSheet * menu=[[UIActionSheet alloc] initWithTitle:@"选择显示时间" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle: nil otherButtonTitles: @"10秒",@"30秒", @"60秒", @"120秒", @"180秒", @"300秒",@"600秒",nil];
...
clickedButtonAtIndex: (NSInteger) buttonIndex{
	...
	switch(buttonIndex)
	{
		...
		case 3:
			...
			break;
		case 4:
			m_dspctl.scantime=180;
			break;
		case 5:
			m_dspctl.scantime=300;
			break;
		case 6:
			m_dspctl.scantime=600;
		case 7://cancel
			return;
	}
	[self ReDraw];
